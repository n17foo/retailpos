...
export class SquarespaceRefundService implements PlatformRefundServiceInterface {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      this.logger.info('Squarespace refund service initialized (mock implementation)');
      return true;
    } catch (error) {
      this.logger.error(
  ...
  async processRefund(orderId: string, refundData: RefundData): Promise<RefundResult> {
    try {
      ...
      // Mock refund processing for Squarespace
      const refundId = `squarespace-refund-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
